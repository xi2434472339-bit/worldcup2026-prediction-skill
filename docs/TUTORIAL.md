...
网站版已通过 OpenRouter 接入 GPT-5.5，**选场次 → 点预测 → 返回严格 JSON 预测卡片**。官方 high 推理可能需要等待后台任务完成。
免费用,不需要注册。

开发仓库中的网站首版已完成本地工程验证：测试、生产构建和 Netlify Functions 加载均通过。正式可用前需要在 Netlify 配置 OpenRouter、API-Football、管理员会话等环境变量。
